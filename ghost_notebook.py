...
def _(dataclass, Config, BatchedTMaze, nn, torch, F, np, RecurrentSNN,
    ...
    class Settings:
        ...
        manager_entropy_weight: float = .001
        prediction_shaping_beta: float = 0.
        encoder_target_tau: float = .005
    ...
    class PredictiveActor(Actor):
        ...
        def forward(self,latent,strategy,context,deterministic=False):
    ...
    class PredictiveAgent(nn.Module):
        def __init__(self,settings):
            ...
            if not 0<settings.encoder_target_tau<=1 or settings.prediction_shaping_beta<0:
                raise ValueError("Invalid target tau or shaping coefficient")
            ...
            self.cfg = Config(seed=settings.seed,worlds=1,encoder_learning_mode="reward_eprop",
                ...
                use_reward_adaln=False,encoder_target_tau=settings.encoder_target_tau)
            cfg = self.cfg
            ...
            self.manager = PredictionManager(cfg)
            ...
            self.management_credit = RewardEprop(list(self.manager.parameters()),settings.slots,
                cfg.strategy_trace_decay,cfg.predictor_lr)
            ...
            # This RNG is independent of policy/manager draws.
        ...
        def reset_episode(self):
            ...
            self.management_credit.reset(torch.ones(k,dtype=torch.bool))
        ...
        def manage(self,z,train):
            ...
            logits = self.manager(self.hidden,z.detach().expand(self.settings.slots,-1),
                self.intents,self.predictions,self.ages.float()[:,None]/self.settings.max_prediction_age)
            dist = torch.distributions.Categorical(logits=logits)
            choice = (dist.sample() if train else logits.argmax(-1)) if self.settings.management else torch.ones_like(self.ages)
            logp = dist.log_prob(choice)*incoming
            entropy = (dist.entropy()*incoming).sum()/incoming.sum().clamp_min(1)
            entropy_grad = None
            if train and self.settings.management:
                self.management_credit.accumulate(logp)
                entropy_grad = torch.autograd.grad(entropy,
                    self.management_credit.parameters,retain_graph=True)
            discard = incoming & (choice==2)
            self.active[discard] = False
            use = incoming & (choice==1)
            ...
            return dict(target=target,use=use,context=context,incoming=incoming,
                choice=choice.detach(),probs=dist.probs.detach(),entropy=float(entropy.detach()),
                entropy_grad=entropy_grad,discard=int(discard.sum()),expired=int(expired.sum()))
        ...
        def supervise(self,observation,train):
            ...
            if not due:
                return []
            with torch.no_grad():
                target=(self.target_encoder if self.settings.use_target_encoder else
                    self.encoder).encode(observation).detach()
            errors=[]
        ...
        def update_target_encoder(self):
            with torch.no_grad():
                for target,online in zip(self.target_encoder.parameters(),self.encoder.parameters()):
                    target.lerp_(online,self.cfg.encoder_target_tau)
...
def _(torch,np,BatchedTMaze):
    def episode(agent,env,train=False):
        ...
        for step in range(episode_time_limit):
            z=agent.encoder.encode(torch.from_numpy(env.observation()))
            ...
            for name,module in [("encoder",agent.encoder),("predictor",agent.predictor),
                    ("management",agent.manager),("critic",agent.critic),
            ...
            with torch.no_grad():
                ...
                external_target=reward+cfg.gamma*next_value
                external_td=(external_target-value.detach()).clamp(-cfg.td_clip,cfg.td_clip)
                actor_signal,next_phi,shaping,correction=agent.actor_reward(reward,cfg.gamma,
            ...
            if train:
                ...
                if s.management:
                    # Same external advantage; separate score eligibility per slot.
                    agent.reward_step(agent.management_credit,external_td.expand(s.slots),
                        managed["entropy_grad"],s.manager_entropy_weight)
            ...
            frames.append(dict(z=z.detach().numpy()[0].copy(),
                ...
                value=float(value.detach()),reward=reward,external_td=float(external_td),
        ...
        report=dict(success=float(successes[0]),cue=cue,episode_return=sum(f["reward"] for f in frames),
...
def _(np,plt):
    ...
    def training_plot(history,window=100,spike_threshold=.15,spike_window=20):
        """Outcome windows restart at stage boundaries; loss windows never do."""
        x=np.arange(1,len(history)+1)
        series={key:[] for key in ("success","left","right","timeout","mse",
            "strategy_entropy","manager_entropy","strategy_std_mean","critic_loss")}
        horizons=sorted({h for r in history for h in r["horizon_accuracy"]})
        horizon_series={h:[] for h in horizons}
        stage_start=0;boundaries=[]
        for i,row in enumerate(history):
            if i and row["curriculum_stage"]!=history[i-1]["curriculum_stage"]:
                stage_start=i;boundaries.append((i+1,row["curriculum_stage"]))
            block=history[max(stage_start,i-window+1):i+1]
            for key in ("success","timeout","strategy_entropy","manager_entropy",
                        "strategy_std_mean"):
        ...
        fig,axes=plt.subplots(6,2,figsize=(14,19),sharex=True,layout="constrained")
        panels=[("Episode outcomes","Rate",[("success","Success"),("left","Left cue"),
                    ("right","Right cue"),("timeout","Timeout")]),
                ("Prediction loss (continuous rolling window)","Validated latent MSE",[("mse","All horizons")]),
                ("Exploration entropy (not calibrated uncertainty)","Entropy (nats)",
                    [("strategy_entropy","Strategy (pre-squash)"),("manager_entropy","Manager")]),
        ...
        axes[2,0].plot(x,[r["critic_loss"] for r in history],label="Raw episode mean (no smoothing)",
            color="tab:orange",marker=".",markersize=3,linewidth=.7,alpha=.7)
        axes[2,0].axhline(spike_threshold,color="firebrick",ls="--",linewidth=.8,label="Spike-density threshold")
        ...
        axes[2,1].step(x,[r["curriculum_stage"] for r in history],where="post",label="Episode stage")
        axes[2,1].plot(x,[max(1.,r["curriculum_stage"]-1+r.get("curriculum_blend_fraction",1.))
            for r in history],label="Distribution blend",color="darkorange")
        axes[2,1].legend(fontsize=8)
        axes[2,1].set(title="Curriculum target and gradual blend",ylabel="Stage",
            yticks=sorted({r["curriculum_stage"] for r in history}))
        axes[3,0].plot(x,[r["strategy_lr_scale"] for r in history],label="Mean LR multiplier")
        axes[3,0].plot(x,[r["strategy_lr_scale_min"] for r in history],label="Minimum LR multiplier",alpha=.6)
        axes[3,0].set(title="Adaptive strategy learning rate (unsmoothed episode summaries)",ylabel="Base LR multiplier",ylim=(0,1.05))
        axes[3,0].legend(fontsize=8)
        for key,label in (("strategy_update_norm","Strategizer"),("encoder_update_norm","Strategy → encoder")):
            axes[3,1].plot(x,[r[key] for r in history],label=label)
        axes[3,1].set(title="Actual parameter updates (episode mean)",ylabel="L2 step norm")
        axes[3,1].legend(fontsize=8)
        trigger_source=history[-1].get("slowdown_trigger_source","prediction")
        axes[4,0].plot(x,[r["prediction_change_score"] for r in history],
            label="Critic deviation" if trigger_source=="critic" else "Median per-horizon deviation")
        axes[4,0].plot(x,[r["prediction_change_threshold"] for r in history],ls="--",color="black",label="Change threshold")
        for episode_index,row in zip(x,history):
            if row["prediction_change_triggered"]:
                axes[4,0].scatter(episode_index,row["prediction_change_score"],color="red",zorder=5)
        axes[4,0].set(title=f"{trigger_source.capitalize()} loss change detector · red = new event",ylabel="Normalized deviation from prior trend")
        axes[4,0].legend(fontsize=8)
        axes[4,1].plot(x,[r["strategy_lr_slowed_fraction"] for r in history])
        axes[4,1].set(title="Training steps slowed by more than 1% (includes recovery)",ylabel="Fraction of steps",ylim=(-.03,1.03))
        density=critic_spike_density(history,spike_threshold,spike_window)
        axes[5,0].plot(x,100*np.asarray(density["fractions"]),color="firebrick")
        axes[5,0].set(title=f"Critic spike density · trailing {spike_window} episodes",
            ylabel=f"Episodes with raw loss > {spike_threshold:g} (%)",ylim=(-2,102))
        axes[5,1].plot(x,density["spike_magnitude"],color="gray",alpha=.6,
            label="Magnitude: mean excess among spike episodes")
        axes[5,1].plot(x,density["density_times_magnitude"],color="darkorange",linewidth=2,
            label="Density × magnitude")
        axes[5,1].set(title="Critic spike density × magnitude · same window",
            ylabel="Excess critic loss",ylim=(0,None))
        ...
        fig.suptitle(f"Training · {len(history)} episodes · rolling {window} episodes; loss history continuous")
        plt.close(fig)
        return fig
    def prediction_loss_plot(history,radius=20):
        """Raw episode means, with validation counts and stage annotations only."""
        horizons=sorted({h for row in history for h in row["horizon_accuracy"]})
        x=np.arange(1,len(history)+1)
        boundaries=[i+1 for i in range(1,len(history))
            if history[i]["curriculum_stage"]!=history[i-1]["curriculum_stage"]]
        views=[("All training episodes",0,len(history))]
        for boundary in boundaries:
            stage=history[boundary-1]["curriculum_stage"]
            views.append((f"Stage {stage} begins at episode {boundary} · ±{radius} episodes",
                max(0,boundary-1-radius),min(len(history),boundary+radius)))
        fig,axes=plt.subplots(len(views),2,figsize=(14,3.4*len(views)),
            squeeze=False,layout="constrained")
        for (title,start,stop),(loss_ax,count_ax) in zip(views,axes):
            selected=history[start:stop];episodes=x[start:stop]
            for h in horizons:
                entries=[row["horizon_accuracy"].get(h,{"count":0,"mse_sum":0.}) for row in selected]
                counts=[entry["count"] for entry in entries]
                losses=[entry["mse_sum"]/entry["count"] if entry["count"] else np.nan for entry in entries]
                line,=loss_ax.plot(episodes,losses,marker=".",markersize=4,linewidth=.8,label=f"Horizon {h}")
                count_ax.plot(episodes,counts,color=line.get_color(),linewidth=.8,label=f"Horizon {h}")
            loss_ax.set(title=title,ylabel="Episode mean validated latent MSE")
            count_ax.set(title="Validated forecasts per episode",ylabel="Count",ylim=(0,None))
            for ax in (loss_ax,count_ax):
                for boundary in boundaries:
                    if start+1<=boundary<=stop:
                        ax.axvline(boundary,color="black",ls=":",alpha=.6)
                ax.set_xlabel("Completed training episode")
                ax.grid(alpha=.2);ax.legend(fontsize=8,ncol=3)
        fig.suptitle("Unsmoothed prediction loss by horizon · gaps = no validated targets · dotted lines = stage changes")
        plt.close(fig)
        return fig
    return training_plot,prediction_loss_plot,critic_spike_density
...
def _(mo):
    ...
    Strategy and management use **external TD only**. The critic observes the
    ...
    Scheduled targets use the EMA encoder. Forecasts never change after creation.
    ...
    **Critic spike density:** counts episodes whose raw mean critic loss exceeds
    the chosen fixed threshold (default 0.15), divided by the number of available
    episodes in the trailing window (default 20). Consecutive high-loss episodes
    each count once; this is episode frequency, not a local-peak count or spikes
    per environment step. Windows never reset at stage changes. The neighboring
    combined panel uses density as a fraction (0–1) multiplied by the mean excess
    loss among spike episodes. This equals the mean excess loss across all episodes
    in the window (the quantity previously labeled magnitude). Gray shows spike-only
    magnitude; orange shows density × magnitude. Both are zero with no spikes.
    These display controls do not change the learning-rate detector.
    ...
    **next episode**. Protection lasts at least the hold duration and until the
    ...
    actor, and manager rates retain their configured values. History continues
    ...
    **Shaping correction:** a prediction is fixed across each physical transition.
    ...
    setup=mo.ui.dictionary({
        ...
        "critic_spike_threshold":mo.ui.number(value=.15,start=0.,step=.01,label="Critic spike-density threshold (raw episode loss)"),
        "critic_spike_window":mo.ui.number(value=20,start=1,stop=1000,step=1,label="Critic spike-density window (episodes)"),
        "plot_window":mo.ui.number(value=100,start=1,stop=1000,label="Plot rolling window (episodes per stage)"),
        ...
        "manager_entropy_weight":mo.ui.number(value=.001,start=0.,stop=.1,step=.001,label="Manager entropy weight"),
        "prediction_shaping_beta":mo.ui.number(value=0.,start=0.,stop=10.,step=.1,label="Actor shaping beta (0 = off)"),
        "encoder_target_tau":mo.ui.number(value=.005,start=.0001,stop=1.,step=.001,label="Target encoder EMA tau"),
        ...
        "management":mo.ui.checkbox(value=False,label="Learn management (off = all eligible traces USE)"),
        ...
        "use_target_encoder":mo.ui.checkbox(value=True,label="EMA prediction targets (off = online encoder)"),
        "adaptive_strategy_lr":mo.ui.checkbox(value=False,label="Slow strategy learning on loss spikes"),
...
@app.cell
def _(setup,mo,Settings,PredictiveAgent,BalancedMaze,torch,episode,summarize,training_plot,prediction_loss_plot):
    mo.stop(setup.value is None,mo.md("Submit to start. Four episodes is a smoke test."))
    _values={k:v for k,v in setup.value.items() if k not in ("episodes","report_every","horizons","plot_every","plot_window","critic_spike_threshold","critic_spike_window")}
        ...
        with mo.status.progress_bar(total=int(setup.value["episodes"]),title="Training episodes",
            ...
            for _ep in range(int(setup.value["episodes"])):
                ...
                if (_ep+1)%int(setup.value["report_every"])==0 or _ep+1==int(setup.value["episodes"]):
                    ...
                    mo.output.append(mo.vstack([mo.md(f"Report through episode {_ep+1}"),
                        mo.ui.table([{k:v for k,v in _summary.items() if not isinstance(v,list)}]),
                        mo.ui.table(_summary["accuracy_by_horizon"]),
                        mo.ui.table(_summary["separation_by_age"]),
                        mo.ui.table(_summary["success_by_hallway_length"]),
                        mo.ui.table(_summary["hallway_distribution_by_stage"])]))
                else:_bar.update()
                if _ep==0 or (_ep+1)%int(setup.value["plot_every"])==0 or _ep+1==int(setup.value["episodes"]):
                    _live_summary=summarize(history[-int(setup.value["plot_window"]):])
                    mo.output.replace(mo.vstack([
                        mo.md(f"Training: **{_ep+1}/{int(setup.value['episodes'])} episodes**. "
                            "Dotted lines mark curriculum changes; outcome windows restart, but critic and prediction loss windows stay continuous. Orange critic points are unsmoothed episode means. "
            ...
            float(setup.value["critic_spike_threshold"]),int(setup.value["critic_spike_window"])),
                        prediction_loss_plot(history),
                        mo.ui.table([{k:v for k,v in interval_reports[-1].items() if not isinstance(v,list)}])
                            if interval_reports else mo.md("First interval report pending."),
                        mo.md("Hallway diagnostics · completed episodes in the current plot window"),
                        mo.ui.table([{k:_live_summary[k] for k in ("mean_hallway_length",
                            "min_hallway_length","max_hallway_length","worst_cue_success",
                            "worst_length_success","worst_length_min_samples","curriculum_blend_fraction")}]),
                        mo.ui.table(_live_summary["sampling_distribution"]),
                        mo.ui.table(_live_summary["success_by_hallway_length"]),
                        mo.ui.table(_live_summary["hallway_distribution_by_stage"])]))
        with torch.no_grad():
    ...
    mo.vstack([training_plot(history,int(setup.value["plot_window"]),
        ...
        mo.ui.table([{k:v for k,v in summary.items() if not isinstance(v,list)}]),
        mo.ui.table(summary["accuracy_by_horizon"]),mo.ui.table(summary["separation_by_age"]),
        mo.ui.table(summary["success_by_hallway_length"]),
        mo.ui.table(summary["hallway_distribution_by_stage"]),
        mo.ui.table(summary["sampling_distribution"])])
    return model,config,history,interval_reports,summary,evaluation,frames
...
@app.cell
def _(mo,frames):
    timestep=mo.ui.slider(0,len(frames)-1,value=0,step=1,show_value=True,label="Evaluation timestep")
    timestep
    return (timestep,)


@app.cell
def _(frames,timestep,mo):
    _f=frames[timestep.value]
    mo.ui.table([dict(slot=i,active=bool(_f["active"][i]),age=int(_f["ages"][i]),
        horizon=int(_f["horizons"][i]),created_at=int(_f["ids"][i]),
        decision=["WAIT","USE","DISCARD"][_f["management"][i]] if _f["managed_active"][i] else "inactive",
        used_this_transition=bool(_f["used"][i])) for i in range(len(_f["active"]))])
    return


@app.cell
def _(frames,timestep,plt,np):
    _f=frames[timestep.value]
    _fig=plt.figure(figsize=(16,13),layout="constrained")
    _gs=_fig.add_gridspec(4,3,height_ratios=[.9,1,1,1])
    _dag=_fig.add_subplot(_gs[0,:]);_dag.axis("off");_dag.set(xlim=(-.5,5.5),ylim=(-.5,2))
    _nodes={"env":(0,1,"Observation"),"encoder":(1,1,"Online encoder"),
        "strategy":(2.5,1,"Stochastic strategy"),"actor":(4,1,"Actor"),"next":(5,1,"Next state"),
        "bank":(2.5,0,"Behavioral bank / manager"),"predict":(4,0,"Predictor / scheduled queue"),
        "critic":(1,0,"External V (pre-decision)"),
        "target":(5,0,"EMA encoder target")}
    for _a,_b in [("env","encoder"),("encoder","strategy"),("encoder","actor"),("strategy","actor"),
                   ("bank","actor"),("actor","next"),("strategy","predict"),("actor","predict"),
                   ("predict","bank"),("encoder","critic"),("bank","critic"),
                   ("next","target"),("target","predict")]:
        _dag.annotate("",xy=_nodes[_b][:2],xytext=_nodes[_a][:2],
            arrowprops=dict(arrowstyle="->",color="slategray",shrinkA=25,shrinkB=25),zorder=1)
    for _x,_y,_label in _nodes.values():
        _dag.text(_x,_y,_label,ha="center",va="center",fontsize=9,
            bbox=dict(boxstyle="round",facecolor="white",edgecolor="gray"),zorder=5)
    _dag.set_title("External TD → strategy / manager; target-encoder prediction error → predictor; shaped reward → actor")
    _axes=[_fig.add_subplot(_gs[1+i//3,i%3]) for i in range(9)]
    _maze=np.zeros((9,9));_maze[1:8,4]=1;_maze[1,1:8]=1
    _axes[0].imshow(_maze,cmap="Greys_r");_axes[0].plot(*_f["pose"],"ro")
    _axes[0].set_title(f"Maze · cue {['left','right'][_f['cue']]}")
    _axes[1].bar(range(len(_f["z"])),_f["z"]);_axes[1].set_title("Encoder latent")
    _axes[2].imshow(np.ma.array(_f["predictions"],mask=np.broadcast_to(~_f["active"][:,None],
        _f["predictions"].shape)),aspect="auto",vmin=-1.5,vmax=1.5,cmap="coolwarm")
    _axes[2].set_title("Immutable behavioral predictions")
    _axes[3].bar(range(len(_f["strategy"])),_f["strategy"]);_axes[3].set_title("Strategy readout (evaluation mean)")
    _axes[4].imshow(np.ma.array(_f["probs"],mask=np.broadcast_to(~_f["managed_active"][:,None],
        _f["probs"].shape)),aspect="auto",vmin=0,vmax=1,cmap="Blues")
    _axes[4].set_xticks([0,1,2],["WAIT","USE","DISCARD"]);_axes[4].set_title("Behavioral management")
    _axes[5].plot([f["actor_signal"] for f in frames],label="actor signal")
    _axes[5].plot([f["reward"] for f in frames],label="external reward")
    _axes[5].axvline(timestep.value,color="red");_axes[5].legend();_axes[5].set_title("Learning-signal separation")
    _axes[6].plot([f["value"] for f in frames]);_axes[6].axvline(timestep.value,color="red")
    _axes[6].set_title("External return baseline")
    _axes[7].bar([0,1,2],_f["actor"]);_axes[7].set_xticks([0,1,2],["left","right","forward"])
    _axes[7].set_ylim(0,1);_axes[7].set_title("Actor probabilities")
    _axes[8].plot([f["queue_length"] for f in frames]);_axes[8].axvline(timestep.value,color="red")
    _axes[8].set_title("Pending mandatory supervision records")
    _fig.suptitle(f"Ghost v6 · timestep {timestep.value}")
    plt.close(_fig)
    _fig
    return
...
@app.cell
def _(mo):
    spike_module=mo.ui.dropdown(["encoder","predictor","management","critic","strategizer","actor"],
...
@app.cell
def _(frames,timestep,spike_module,plt):
    _raster,_ax=plt.subplots(figsize=(12,2.5),layout="constrained")
    _ax.imshow(frames[timestep.value]["spike_rates"][spike_module.value],
