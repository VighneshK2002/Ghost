...
def _(dataclass):
    ...
    class Config:
        ...
        use_action_cost: bool = True
        action_cost: float = 0.002
        use_overshoot_penalty: bool = True
        ...
        predictor_latent_only: bool = False
...
def _(Config, NamedTuple, RecurrentSNN, mlp, nn, torch):
    class Predictor(nn.Module):
        ...
        def __init__(self, cfg: Config) -> None:
            ...
            self.norm = nn.LayerNorm(2*cfg.hidden_dim)
            if cfg.predictor_latent_only:
                self.latent_head = nn.Linear(
                    2*cfg.hidden_dim, cfg.latent_dim)
            else:
        ...
        def forward(self, latent, ghost, relative_horizon, budget_fraction):
            ...
            if self.cfg.predictor_latent_only:
                return self.latent_head(feature)
...
def _(Config, dataclass, np):
    class PointControl:
        def __init__(self, cfg: Config, seed: int) -> None:
            ...
            initial_stage = (
                ...
                else self.curriculum_stage_count-1)
            self.curriculum_stage = int(np.clip(
                initial_stage, 0, self.curriculum_stage_count-1))
    ...
    class TaskRewardPointControl(PointControl):
        ...
        def step(self, action: np.ndarray):
            ...
            shaping = np.zeros(self.cfg.worlds, np.float32)
            if self.cfg.use_progress_shaping:
                shaping += self.cfg.progress_weight*progress
            if self.cfg.use_action_cost:
                shaping -= self.cfg.action_cost*np.square(action)
            if self.cfg.use_overshoot_penalty:
                shaping -= (
...
def _(Config, mo):
    ...
    training_form = mo.ui.dictionary({
        ...
        "use_progress_shaping": mo.ui.switch(
            value=defaults.use_progress_shaping,
            label="Reward · Enable progress shaping"),
        "progress_weight": number(
            defaults.progress_weight,
            "Reward · Progress weight (when enabled)",
            step=0.1, start=0.0),
        "use_action_cost": mo.ui.switch(
            value=defaults.use_action_cost,
            label="Reward · Enable action cost"),
        "action_cost": number(
            defaults.action_cost,
            "Reward · Action cost (when enabled)",
            step=0.001, start=0.0),
        "use_overshoot_penalty": mo.ui.switch(
            value=defaults.use_overshoot_penalty,
            label="Reward · Enable overshoot penalty"),
        "overshoot_penalty": number(
            defaults.overshoot_penalty,
            "Reward · Overshoot penalty (when enabled)",
            step=0.01, start=0.0),

        "use_curriculum": mo.ui.switch(
            value=defaults.use_curriculum,
            label="Curriculum · Enable staged training"),
        "curriculum_stage": number(
            defaults.curriculum_stage,
            "Curriculum · Initial stage (when enabled)",
            step=1, start=0,
        ...
        "actual_horizon_min_probability": number(
            ...
            step=0.01, start=0.0, stop=1.0),
        "predictor_latent_only": mo.ui.switch(
            value=defaults.predictor_latent_only,
            label="Predictor · Latent-only MSE mode"),
        "predictor_logvar_min": number(
            defaults.predictor_logvar_min,
            "Predictor · Minimum log variance (full mode)", step=0.1),
        "predictor_logvar_max": number(
            defaults.predictor_logvar_max,
            "Predictor · Maximum log variance (full mode)", step=0.1),
        "return_loss_weight": number(
            defaults.return_loss_weight,
            "Predictor · Return loss weight (full mode)",
            step=0.1, start=0.0),
...
def _(
    ...
    initial_uncertainty = 0.0 if cfg.predictor_latent_only else 1.0
    previous_uncertainty = torch.full(
        (b,), initial_uncertainty, device=device)
    ...
    while transitions < cfg.transitions:
        ...
        if cfg.predictor_latent_only:
            predicted_endpoint = predictor_output
            uncertainty = torch.zeros(b, device=device)
        else:
            predicted_endpoint, predicted_logvar, predicted_return = (
                predictor_output)
            uncertainty = predicted_logvar.exp().mean(-1).sqrt()
        with torch.no_grad():
            endpoint_value = target_critic(
                predicted_endpoint.detach(), ghost_intent.detach())
            discount = torch.pow(
                torch.full_like(endpoint_value, cfg.gamma),
                actual_horizon.float())
            if cfg.predictor_latent_only:
                forecast_value = discount*endpoint_value
            else:
                forecast_value = (
                    predicted_return.detach()+discount*endpoint_value)
        ...
        with torch.no_grad():
            ...
            next_ghost_intent = query_next_ghost_intent(
                endpoint_target_latent, predicted_endpoint.detach(),
        ...
        for parameter, control_grad in zip(
                actor_trace.parameters, actor_control_gradients):
            parameter.grad = control_grad.detach().clone()
        ...
        for world, record in matured:
            ...
            if cfg.predictor_latent_only:
                prediction_loss = F.mse_loss(replay_output, target)
            else:
                replay_mean, replay_logvar, replay_return = replay_output
                squared = (target-replay_mean).square()
                nll = 0.5*(
                    squared*torch.exp(-replay_logvar)
                    +replay_logvar).mean()
                return_target = torch.tensor(
                    [record.discounted_return], device=device)
                return_loss = F.mse_loss(replay_return, return_target)
                prediction_loss = (
                    nll+cfg.return_loss_weight*return_loss)
        ...
        previous_endpoint = predicted_endpoint.detach()
        ...
        if bool(done.any()):
            ...
            previous_uncertainty[done] = initial_uncertainty
        ...
        if (cfg.use_curriculum
                and len(stage_history) == stage_history.maxlen
            ...
            previous_uncertainty.fill_(initial_uncertainty)
